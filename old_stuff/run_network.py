...
class DatasetReader:
    ...
    def decode_features(self,fullExample):    
        ...
        image = tf.reshape(image, [disp_height,disp_width, 3])
        disp = tf.reshape(disp, [disp_height,disp_width])
        opt_flow = tf.reshape(opt_flow, [opt_flow_height,opt_flow_width])
        disp_chng = tf.reshape(disp_chng,[disp_chng_height,disp_chng_width])
