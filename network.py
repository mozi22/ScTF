...
def network_tunnel(image,var_scope_name):
    ...
    with tf.variable_scope(var_scope_name):
        ...
        conv3 = convrelu2(name='conv3', inputs=conv2_1, filters=64, kernel_size=3, stride=1,activation=myLeakyRelu)
...
def network_core(tunnel_layer1,tunnel_layer2):
    ...
    with tf.variable_scope('network_core'):
        ...
        conv5 = convrelu2(name='conv5', inputs=conv4_1, filters=256, kernel_size=3, stride=2,activation=myLeakyRelu)
        conv5_1 = convrelu2(name='conv5_1', inputs=conv5, filters=256, kernel_size=3, stride=1,activation=myLeakyRelu)

        conv6 = convrelu2(name='conv6', inputs=conv5_1, filters=512, kernel_size=3, stride=2,activation=myLeakyRelu)
    ...
    with tf.variable_scope('refine4'):
        concat3 = _refine(
            inp=concat4, 
            num_outputs=128,
            features_direct=correlated_layer
        )
    ...
    with tf.variable_scope('predict_flow2'):
        ...
        predict_flow2 = _predict_flow(concat3)
