...
def train_network(image_pair):
	...
	# predict flow
	with tf.variable_scope('predict_flow5'):
		predict_halfway_flow = convrelu2(name='conv5_flow', inputs=conv5_flow, filters=24, kernel_size=3, stride=1)


	print(predict_halfway_flow)

	with tf.variable_scope('upsample_flow5to4'):
		predict_flowconf5to4 = _upsample_prediction(predict_halfway_flow, 2)

	print(predict_flowconf5to4)